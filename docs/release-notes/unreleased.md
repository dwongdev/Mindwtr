...
- fix(desktop): attaching a file the app can't read (for example from a RAM drive or other restricted location) now shows an error right away instead of silently adding an attachment that can never be opened. (#836)
