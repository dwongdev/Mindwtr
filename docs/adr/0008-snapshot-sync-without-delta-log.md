...
For a personal GTD app at current scale, the snapshot approach is still the right trade-off:

- per-user datasets are typically small
...
Mindwtr keeps snapshot-based sync and does not add a delta log at this time.
...
- We should revisit this decision only if one or more of these thresholds are crossed:
  - a typical user snapshot exceeds 5 MB
  - sync round-trip latency exceeds 5 seconds on a typical network
  - Mindwtr needs real-time multi-device streaming behavior
- If that revisit happens, the first design to evaluate is an append-only `mindwtr-delta.jsonl` alongside `mindwtr-snapshot.json`, compacted by keeping the highest revision per entity id and tracking per-device watermarks.
