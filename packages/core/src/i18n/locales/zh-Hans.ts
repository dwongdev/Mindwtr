...
export const zhHans: Record<string, string> = {
        ...
        'settings.syncFailureAuth': '请在“数据与同步”中重新验证或检查同步凭据。',
        'settings.syncFailurePermission': '请在“数据与同步”中重新选择同步文件或文件夹，或重新授予访问权限。',
        'settings.syncFailureRateLimited': '同步后端正在限流。请稍后再试。',
        'settings.syncFailureMisconfigured': '请先在“数据与同步”中完成所选同步后端的配置。',
        'settings.syncFailureConflict': '另一台设备或后端报告了同步冲突。请等待双方完成同步后再重试。',
        'settings.syncFailureGeneric': '请检查“数据与同步”设置后重试。',
        ...
        'settings.syncConflictNotice': '同步冲突已按最后写入优先自动解决。打开“数据与同步”查看详情。',
